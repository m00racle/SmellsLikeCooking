...
public class DirectionsFragment extends Fragment {
    ...
    private static final String KEY_CHECKED_DIRECTIONS = "key_checked_directions";
    private CheckBox[] checkBoxes;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        //: retreive index Argument from the ViewPagerFragment:
        int index = getArguments() != null ? getArguments().getInt(ViewPagerFragment.KEY_RECIPE_INDEX) : 0;

        View view = inflater.inflate(R.layout.fragment_directions, container, false);

        //: get the String array of directions from the model Recipe class:
        //  then use regex to separate the String into String array
        String[] directions = Recipes.directions[index].split("`");

        //: initialize checkboxes related to the size of the directions String array length:
        ...
        //  else the checkedDirections will still instantiated but by default all value will be all false
        boolean[] checkedDirections = new boolean[checkBoxes.length];
        if (savedInstanceState != null && savedInstanceState.getBooleanArray(KEY_CHECKED_DIRECTIONS) != null){
            checkedDirections = savedInstanceState.getBooleanArray(KEY_CHECKED_DIRECTIONS);
        }

        //: with all set for creating checkBoxes for all directions steps we can set it into our chosen layout:
        LinearLayout directionLayout = view.findViewById(R.id.directionLayout);

        //: loop through all directions steps and set it onto the checkboxes then set the toggle if checked or not:
        setUpDirectionCheckBox(directions, checkedDirections, directionLayout);

        return view;
    }

    private void setUpDirectionCheckBox(String[] directions, boolean[] checkedDirections, @Nullable ViewGroup container) {
        int i = 0;
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        //: put the boolead array data into bundle object called outState:
        outState.putBooleanArray(KEY_CHECKED_DIRECTIONS, stateOfCheckBoxes);
